/**
 * Composition root for example 03 (Hermes — Nous Research). Runs a manager that
 * spawns Hermes peers (extensions/hermes-py) as detached processes via uv. Each peer
 * embeds Hermes' AIAgent plus a Python Swarl wire client and answers DMs, anycasts,
 * and @-mentions on channels — interoperating on the wire with the TypeScript peers.
 */
...
import { hermesConnector } from "@swarl/hermes-py";
...
const swarlAlias: Connector = {
  kind: "connector",
  name: "swarl",
  buildLaunch: hermesConnector.buildLaunch,
};
registry.register(swarlAlias);

const mgr = new Manager({ space, registry, servers: server, spawnMode: "detached" });
await mgr.start();
console.log(`example-03-hermes-py manager up in space "${space}" — spawn mode: detached`);
