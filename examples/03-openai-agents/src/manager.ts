 ...
 * (extensions/openai-agents) that embeds a Swarl endpoint and answers DMs, anycasts,
 * and @-mentions on channels. Importing the connector self-registers it as
 * "openai-agents"; we also alias it under "swarl" so a bare `swarl start --name x`
...
import { openaiAgentsConnector } from "@swarl/openai-agents"; // self-registers "openai-agents"
...
await mgr.start();
console.log(`example-03-openai-agents manager up in space "${space}" — connectors: openai-agents, swarl`);
