// ==UserScript==
// @name         Cotal Face — OpenCode avatar
// @namespace    cotal
// @version      0.1.0
// @description  Mounts an animated pixel-art cotal-face on OpenCode's project avatar tile.
...
// ==/UserScript==
//
// GENERATED: the engine block below is cotal-face.js verbatim (the source of truth).
// Edit cotal-face.js, then re-sync this bundle. Userscript-only mount logic is at the bottom.
...
// ---- OpenCode avatar auto-mount (userscript-only) -------------------------------------------
// OpenCode's web UI is a SolidJS SPA with no plugin hook for the avatar slot, and its CSP blocks
// injected <script> tags — but a userscript manager runs privileged, so this works. The engine
// above uses only customElements.define + DOM APIs (no eval/fetch), so CSP does not restrict it.
(function () {
  const PERSONA = 'ray'; // ray | david | neon

  // OpenCode's project avatar tile: a small (~32px) rounded "overflow-clip" box, top-left rail.
  function findTile() {
    const els = document.querySelectorAll('[class*="overflow-clip"]');
    for (const el of els) {
      const r = el.getBoundingClientRect();
      if (r.width >= 24 && r.width <= 46 && r.top < 130 && r.left < 90) return el;
    }
    return null;
  }
  ...
  function mount() {
    const tile = findTile();
    if (!tile) { if (overlay) { overlay.remove(); overlay = null; } return; }
    const r = tile.getBoundingClientRect();
    if (!overlay) {
      overlay = document.createElement('div');
      overlay.id = 'cotal-face-overlay';
      const f = document.createElement('cotal-face');
      f.setAttribute('persona', PERSONA);
      f.setAttribute('chrome', 'off');
      f.setAttribute('crt', 'off');
      overlay.appendChild(f);
      document.body.appendChild(overlay);
    }
    overlay.style.cssText =
      'position:fixed;z-index:99999;border-radius:8px;overflow:hidden;pointer-events:none;' +
      'left:' + r.left + 'px;top:' + r.top + 'px;width:' + r.width + 'px;height:' + r.width + 'px;' +
      '--cf-width:' + r.width + 'px';
  }

  // OpenCode re-renders the tile per route; keep the overlay aligned.
