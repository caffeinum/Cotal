 ...
 * Vercel-AI-SDK peers into the space as detached processes. Each spawn is a real
 * agent loop (extensions/vercel-ai) that embeds a Swarl endpoint and answers DMs,
 * anycasts, and @-mentions on channels via generateText + tool calls.
 */
...
import { vercelAiConnector } from "@swarl/vercel-ai";
...
await mgr.start();
console.log(`example-03-vercel-ai manager up in space "${space}" — spawn mode: detached`);
