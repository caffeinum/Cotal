...
The launcher (``src/launch.ts``) owns the mesh endpoint and exposes a unix-socket bridge; this is
...
class BridgeClient:
    ...
    def call_tool(self, op: str, args: dict, timeout: float = 30.0) -> Any:
        """Invoke a cotal op on the sidecar and block for its result (raises on error/timeout)."""
        rid = uuid.uuid4().hex
        ev = threading.Event()
        box: dict = {}
        self._pending[rid] = (ev, box)
        try:
            self._send({"t": "tool", "id": rid, "op": op, "args": args})
            if not ev.wait(timeout):
                raise TimeoutError(f"cotal tool '{op}' timed out")
            if not box.get("ok"):
...
def get_client() -> BridgeClient:
    """Process-wide singleton, bound to ``COTAL_BRIDGE_SOCKET`` (set by the launcher)."""
    global _client
    if _client is None:
        path = os.environ.get("COTAL_BRIDGE_SOCKET")
        if not path:
            raise RuntimeError(
                "COTAL_BRIDGE_SOCKET not set — the Cotal launcher must spawn the gateway"
            )
