...
export function serverConfig(auth: SpaceAuth, opts: { port?: number; storeDir: string }): string {
  ...
  // the connection (the client retries forever — a connect that "hangs"). Raise the limit so a
  // rich agent JWT always fits. It bounds a single protocol line (buffer grown on demand), so a
  // generous cap costs nothing at rest.
  return `# Generated by \`cotal up\` — do not edit by hand.
port: ${port}
max_control_line: 1048576
