...
import { SwarlEndpoint } from "./endpoint.js";
 ...
 * name-based peer resolution. This is the shared core behind every framework
 * adapter (the Claude MCP server, the OpenAI/Vercel embedded peers) — it owns the
 * NATS connection and presence so the adapter only wires the model loop.
 *
 * Connecting is resilient: {@link start} kicks off a background retry loop so the
 * adapter is responsive immediately even if the mesh isn't up yet.
 *
 * Emits `"incoming"` (InboxItem) after each message is buffered, so a push layer
 * (a channel, or an agent run loop) can deliver it immediately; `"error"` (Error)
 * for endpoint faults.
 */
export class MeshAgent extends EventEmitter {
  ...
  private log(msg: string): void {
    process.stderr.write(`[swarl-mesh] ${msg}\n`);
  }
}
