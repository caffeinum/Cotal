...
Each failed step offers an interactive Claude handoff (skippable with `COTAL_SKIP_ASSIST=1`) that
carries the failure context and resumes setup on `/exit`. Adds `cotal up --detach` + `cotal down` for a
