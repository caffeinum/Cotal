...
import { registry, type Runtime, type RuntimeKind, type RuntimeProvider } from "@cotal/core";
import { PtyRuntime } from "./pty.js";
import { TmuxRuntime, tmuxAvailable } from "./tmux.js";

export type { Runtime, RuntimeKind, AgentHandle, AttachSession } from "@cotal/core";
 ...
 *  (e.g. `@cotal/cmux`). `session` names the tmux session (per space) for tmux. */
export function createRuntime(mode: RuntimeMode, session: string): Runtime {
  ...
  try {
    ...
    throw new Error(
      `unknown runtime "${kind}" — is its integration imported? (e.g. import "@cotal/cmux")`,
    );
  }
