...
| cmux session | `CMUX_SURFACE_ID` gate + `cmuxManagerRunning`/`pgrepMatches` ([`lib/manager-proc.ts`](../implementations/cli/src/lib/manager-proc.ts)) + the **`Workspaces`** contract ([`packages/core/src/runtime.ts`](../packages/core/src/runtime.ts)) and its `cmux` provider ([`extensions/cmux/src/runtime.ts`](../extensions/cmux/src/runtime.ts)) | `ensureCmuxSession` resolves the `cmux` **`Workspaces`** capability from the core registry (`registry.resolve("workspaces","cmux")`, registered by the composition root's `import "@cotal-ai/cmux"`) to open/close tabs — so the CLI never imports the extension. It opens a `cotal cmux --spawn david,sven` manager tab (owns david/sven so they're despawnable) + a focused `cotal-main` workspace. Gated on the **live process** (not just an open tab, which lingers after its process dies): if none runs it closes stale tabs and opens fresh. The manager staggers david/sven (waits for each to register presence before the next) so cold-starts don't spike memory |
