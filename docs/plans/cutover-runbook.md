# Auth cutover runbook — flipping the demo space to `--auth`
...
- `.swarl/auth` generated (account + signing key) — `up --auth` is load-or-create, so the
...
- **Signing-key continuity.** `swarl up --auth` is **load-or-create**
  ...
  `up --auth` repeatedly reuses the same `.swarl/auth`, so pre-minted creds stay
...
3. `swarl up --auth --space demo` once to generate/confirm `.swarl/auth` (operator/
...
7. Start `swarl up --auth` (operator config + MEMORY resolver; pre-creates streams +
...
11. **CREDLESS CONNECT IS REFUSED** — a raw connect / `swarl watch` with NO creds is
    ...
    if `up --auth` silently came up open, everything below would pass while unprotected.
...
18. Start plain `swarl up` (open `-js`); open-mode lazy-create rebuilds streams in `$G`.
