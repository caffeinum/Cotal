...
- `.cotal/auth` generated (account + signing key) — `up` is load-or-create, so the
  flip reuses this exact key and the pre-minted creds stay valid (invariant #1).
- Per-peer creds pre-minted in `.cotal/auth/creds/` (gitignored, local): `ada.creds`,
  `dave.creds`, `david.creds`, `linus.creds` (agent profiles, channel scope from each agent
  file) + `dashboard.creds` (observer). Hand each peer its file (`--creds <path>` / `COTAL_CREDS`)
...
- **Signing-key continuity.** `cotal up` is **load-or-create**
  ...
  `up` repeatedly reuses the same `.cotal/auth`, so pre-minted creds stay
  valid. Never regenerate `.cotal/auth` between minting creds and starting the server.
- **Secrets hygiene.** `.cotal/auth/**` (incl. `creds/`) is gitignored — verify it
...
> **Rehearse in a SEPARATE workspace dir** (or `rm -rf .cotal/auth` at teardown):
> `.cotal/auth` is per-*workspace*, not per-space — one `auth.json` holding a single
...
3. `cotal up --space demo` once to generate/confirm `.cotal/auth` (operator/
   ...
   the real start is step 7). *(Idempotent — safe even if `.cotal/auth` already exists.)*
4. Pre-mint creds against that signing key: `cotal mint <name> --profile observer`
...
7. Start `cotal up` (operator config + MEMORY resolver; pre-creates streams +
   KV in the demo account).
8. Start the manager (with `.cotal/auth` present → mints its own manager-profile creds,
...
11. **CREDLESS CONNECT IS REFUSED** — a raw connect / `cotal watch` with NO creds is
...
12. `cotal ps --creds <c>` lists agents — control plane alive under auth.
...
18. Start `cotal up --open` (unauthenticated `-js`); open-mode lazy-create rebuilds streams in `$G`.
