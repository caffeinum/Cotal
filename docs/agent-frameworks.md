...
built with general-purpose SDKs can join a Swarl space as native lateral peers. Two
adapters ship today:
...
bridge. The shared piece is `MeshAgent` (in `@swarl/connector-core`, the same runtime
...
2. **Mesh awareness as tools.** The model also gets read/presence tools via the framework's
   tool mechanism (`tool()` for both SDKs): `swarl_roster` (who's present) and `swarl_status`
...
peer via `tsx` and forwards the launcher's identity (`SWARL_ID`), minted creds
(`SWARL_CREDS`), and any agent file (`SWARL_AGENT_FILE`), so under auth the peer
